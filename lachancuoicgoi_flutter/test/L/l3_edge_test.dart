...
void main() {
  ...
  group('L3Analyzer - edge cases', () {
    ...
    test('analyzeIncremental with null session returns null', () async {
    ...
    test('analyzeIncremental with zero new text returns last result', () async {
      final analyzer = _createL3AnalyzerWithSession();
    ...
    test(
      ...
      () async {
        ...
        final result = await analyzer.analyzeIncremental('xin chao cac ban');
        expect(result, isNull);
      },
    );

    test('risk decay: 3 consecutive greens de-escalate from red', () async {
      final analyzer = _createL3AnalyzerWithSession();
    ...
    test('risk decay: green interrupted by orange resets counter', () async {
      final analyzer = _createL3AnalyzerWithSession();
      ...
      expect(
        orange.overallRiskLevel,
        RiskLevel.orange,
      ); // returns parsed level, not max
    ...
    test('cache miss triggers API call for different text', () async {
      var requestCount = 0;
      final analyzer = _createL3AnalyzerWithClient(
        requestExecutor:
            ({
    ...
    test('analyze with empty text returns green without API call', () async {
      var requestCount = 0;
      final analyzer = _createL3AnalyzerWithClient(
        requestExecutor:
            ({
    ...
    test('analyze with too few words returns green without API call', () async {
      var requestCount = 0;
      final analyzer = _createL3AnalyzerWithClient(
        requestExecutor:
            ({
    ...
    test('createSession closes previous session', () async {
      final analyzer = _createL3AnalyzerWithSession();
      analyzer.createSession();
      analyzer.syncProcessedTextLength(50);
    ...
    test('parseResponse with blank response throws FormatException', () async {
      final analyzer = _createL3AnalyzerWithSession();
    ...
    test('parseResponse with null/unknown level infers from reason', () async {
      final analyzer = _createL3AnalyzerWithSession();
    ...
    test('resetSession delegates to closeSession', () async {
      final analyzer = _createL3AnalyzerWithSession();
      analyzer.createSession();
      analyzer.syncProcessedTextLength(100);
    ...
    test('level returns l3', () async {
      final analyzer = _createL3AnalyzerWithSession();
      expect(analyzer.level, AnalysisLevel.l3);
    });

    test('isReady returns false when no API keys', () async {
    ...
    test('analyze API error sets isError flag', () async {
      final analyzer = _createL3AnalyzerWithClient(
        requestExecutor:
            ({
              required String apiKey,
    ...
    test('consecutive API errors increment error counter', () async {
      final analyzer = _createL3AnalyzerWithClient(
        requestExecutor:
            ({
              required String apiKey,
